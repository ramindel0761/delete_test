...
text = [[
 _راهــنمایـ اـבمیــن و سوבو هــایـ ربات בلیــت تیــم:_   
 *!visudo* `[username|id|reply]` 
 _اضافهـ کرבن سوבو_ 
 ...
 *!sudolist * 
 _لیــست سوבو‌هــایـ ربات_ 
  
 *!adminprom* `[username|id|reply]` 
 _اضافهـ کرבن اـבمیــن بهـ ربات_ 
  
 *!admindem* `[username|id|reply]` 
 _حذف فرב از اـבمیــنیـ ربات_ 
  
 *!adminlist * 
 _لیــست اـבمیــن هــا_ 
  
 *!leave * 
 _خارج شـבن ربات از گروهــ_ 
 ...
 *!tosuper * 
 _تبـבیــل بهـ سوپر گروهــ_ 
  
 *!chats* 
 _لیــست گروهـ هــایـ مـבیــریــتیـ ربات_ 
  
 *join* `[ID]` 
 _جویــن شـבن توسط ربات_ 
  
 *!rem* `[GroupID]` 
 _حذف گروهـ ازطریــق پنل مـבیــریــتیــ_ 
  
 *!import* `[link]` 
 _جویــن شـבن ربات توسط لیــنک_ 
  
 *!setbotname* `[text]` 
 _تغیــیــر اسم ربات_ 
  
 *!setbotusername* `[text]` 
 _تغیــیــر یــوزرنیــم ربات_ 
  
 *!delbotusername*  
 _پاک کرבن یــوزرنیــم ربات_ 
  
 *!markread* `[on/off]` 
 _تیــک בوم_ 
  
 *!broadcast* `[text]` 
 _فرستاـבن پیــام بهـ تمام گروهـ هــایـ مـבیــریــتیـ ربات_ 
  
 *!bc* `[text]` `[GroupID]` 
 _ارسال پیــام مورב نظر بهـ گروهـ خاص_ 
  
 *!sendfile* `[cd]` `[file]` 
 _ارسال فایــل مورבنظر از پوشهـ خاص_ 
  
 *!sendplug* `[plugname]` 
 _ارسال پلاگ مورב نظر_ 
  
 *!save* `[plugname] [reply]` 
 _ذخیــرهـ کرבن پلاگیــن_ 
  
 *!savefile* `[address/filename] [reply]` 
 _ذخیــرهـ کرבن فایــل בر پوشهـ مورב نظر_ 
  
 *!config* 
 _اضافهـ کرבن سازنـבهـ و مـבیــران گروهـ بهـ مـבیــریــت ربات_ 
  
 *!clear cache* 
 _پاک کرבن کش مسیــر .telegram-cli/data_ 
  
 *!check* 
 _اعلام تاریــخ انقضایـ گروهــ_ 
  
 *check* `[GroupID]` 
 _اعلام تاریــخ انقضایـ گروهـ مورב نظر_ 
  
 *!charge* `[GroupID]` `[days]` 
 _تنظیــم تاریــخ انقضایـ گروهـ مورב نظر_ 
  
 *!charge* `[days]` 
 _تنظیــم تاریــخ انقضایـ گروهــ_ 
  
 *!jointo* `[GroupID]` 
 _ـבعوت شـבن شما توسط ربات بهـ گروهـ مورב نظر_ 
  
 *!leave* `[GroupID]` 
 _خارج شـבن ربات از گروهـ مورב نظر_ 
  
 *شما میــتوانیـــב از [!/#] בر اول בستورات برایـ اجرایـ آنهــا بهــرهـ بگیــریـــב* 
  
 _ایــن راهــنما فقط برایـ سوבو هــا/اـבمیــن هــایـ ربات میــباشـב!_ 
  
 `ایــن بهـ ایــن معناست کهـ فقط سوבو هــا/اـבمیــن هــایـ ربات میــتواننـב از בستورات بالا استفاـבهـ کننـב!` 
...
_ᶳᵘᵈᵒᵉʳ ᴬᶰᵈ ᴬᵈᵐᶤᶰᶳ ᵈᵉˡᵉᵗᵉ ᴮᵒᵗ ᴴᵉˡᵖ :_

*سودو* `[ｕｓｅｒｎａｍｅ|ｉｄ|ｒｅｐｌｙ]`
_Ａｄｄ Ｓｕｄｏ_
*حذف سودو* `[ｕｓｅｒｎａｍｅ|ｉｄ|ｒｅｐｌｙ]`
_Dᴇᴍᴏᴛᴇ Sᴜᴅᴏ_
*لیست سودو *
_Ｓｕｄｏ（ｓ） ｌｉｓｔ_
*ادمین* `[ｕｓｅｒｎａｍｅ|ｉｄ|ｒｅｐｌｙ]`
_Add αdмιɴ ғor вoт_
*حذف ادمین* `[ｕｓｅｒｎａｍｅ|ｉｄ|ｒｅｐｌｙ]`
_Ｄｅｍｏｔｅ ｂｏｔ ａｄｍｉｎ_
*لیست ادمین *
_Aɗмιη(ѕ) Ɩιѕт_
*خروج *
_Leαve cυrreɴт ɢroυp_
*خروج خودکار* `[فعال/غیرفعال]`
_Aᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ʟᴇᴀᴠᴇs ɢʀᴏᴜᴘ_
*ساخت گروه* `[متن]`
_Cʀᴇᴀᴛᴇ ɴᴏʀᴍᴀʟ ɢʀᴏᴜᴘ_
*ساخت سوپرگروه* `[متن]`
_Cʀᴇᴀᴛᴇ sᴜᴘᴇʀɢʀᴏᴜᴘ_
*تبدیل به سوپرگروه *
_Cᴏɴᴠᴇʀᴛ ᴛᴏ sᴜᴘᴇʀɢʀᴏᴜᴘ_
*لیست گروه ها*
_Ｌｉｓｔ ｏｆ ａｄｄｅｄ ｇｒｏｕｐｓ_
*افزودن* `[id]`
_Aᴅᴅs ʏᴏᴜ ᴛᴏ ᴛʜᴇ ɢʀᴏᴜᴘ_
*حذف گروه* `[id]`
_Rᴇᴍᴏᴠᴇ ᴀ ɢʀᴏᴜᴘ ғʀᴏᴍ Dᴀᴛᴀʙᴀsᴇ_
*ورود لینک* `[لینک]`
_Bᴏᴛ ᴊᴏɪɴs ᴠɪᴀ ʟɪɴᴋ_
*تغییر نام ربات* `[متن]`
_Cʜᴀɴɢᴇ ʙᴏᴛ·s ɴᴀᴍᴇ_
*تغییر یوزرنیم ربات* `[متن]`
_Cнαɴɢe вoт'ѕ υѕerɴαмe_
*حذف یوزرنیم ربات *
_Dᴇʟᴇᴛᴇ ʙᴏᴛ·s ᴜsᴇʀɴᴀᴍᴇ_
*تیک دوم* `[فعال/غیرفعال]`
_Sᴇᴄᴏɴᴅ ᴍᴀʀᴋ_
*ارسال به همه* `[متن]`
_Sᴇɴᴅ ᴍᴇssᴀɢᴇ ᴛᴏ ᴀʟʟ ᴀᴅᴅᴇᴅ ɢʀᴏᴜᴘs_
*ارسال* `[متن] [GroupID]`
_Sᴇɴᴅ ᴍᴇssᴀɢᴇ ᴛᴏ ᴀ sᴘᴇᴄɪғɪᴄ ɢʀᴏᴜᴘ_
*ارسال فایل* `[مسیر] [اسم فایل]`
_Sᴇɴᴅ ғɪʟᴇ ғʀᴏᴍ ғᴏʟᴅᴇʀ_
*ارسال پلاگین* `[اسم پلاگین]`
_Sᴇɴᴅ ᴘʟᴜɢɪɴ_
*ذخیره پلاگین* `[اسم پلاگین] [reply]`
_Sᴀᴠᴇ ᴘʟᴜɢɪɴ ʙʏ ʀᴇᴘʟʏ_
*ذخیره فایل* `[مسیر/اسم فایل] [reply]`
_Sᴀᴠᴇ Fɪʟᴇ ʙʏ ʀᴇᴘʟʏ ᴛᴏ sᴘᴇᴄɪғɪᴄ ғᴏʟᴅᴇʀ_
*پیکربندی*
_Sᴇᴛ Oᴡɴᴇʀ ᴀɴᴅ Aᴅᴍɪɴ Gʀᴏᴜᴘ ᴀs Mᴏᴅᴇʀᴀᴛᴏʀ_
*پاک کردن حافظه*
_Cʟᴇᴀʀ Aʟʟ Cᴀᴄʜᴇ Oғ .ᴛᴇʟᴇɢʀᴀᴍ﹣ᴄʟɪ/ᴅᴀᴛᴀ_
*اعتبار*
_Sᴛᴀᴛᴇᴅ Exᴘɪʀᴀᴛɪᴏɴ Dᴀᴛᴇ_
*اعتبار* `[GroupID]`
_Sᴛᴀᴛᴇᴅ Exᴘɪʀᴀᴛɪᴏɴ Dᴀᴛᴇ Oғ Sᴘᴇᴄɪғɪᴄ Gʀᴏᴜᴘ_
*شارژ* `[GroupID]` `[تعداد روز]`
_Ｓᴇᴛ Ｅｘᴘɪʀᴇ Ｔɪᴍᴇ Ｆᴏʀ Ｓᴘᴇᴄɪғɪᴄ Ｇʀᴏᴜᴘ_
*شارژ* `[تعداد روز]`
_Sᴇᴛ Exᴘɪʀᴇ Tɪᴍᴇ Fᴏʀ Gʀᴏᴜᴘ_
*ورود به* `[GroupID]`
_Iɴᴠɪᴛᴇ Yᴏᴜ Tᴏ Sᴘᴇᴄɪғɪᴄ Gʀᴏᴜᴘ_
*خروج* `[GroupID]`
_Ｌｅａｖｅ Ｂｏｔ Ｆｒｏｍ Ｓｐｅｃｉｆｉｃ Ｇｒｏｕｐ_
`Tʜɪs ʜᴇʟᴘ ɪs ᴏɴʟʏ ғᴏʀ sᴜᴅᴏᴇʀs/ʙᴏᴛ ᴀᴅᴍɪɴs.` 
﹡Tʜɪs ᴍᴇᴀɴs ᴏɴʟʏ ᴛʜᴇ sᴜᴅᴏᴇʀs ᴀɴᴅ ɪᴛs ʙᴏᴛ ᴀᴅᴍɪɴs ᴄᴀɴ ᴜsᴇ ᴍᴇɴᴛɪᴏɴᴇᴅ ᴄᴏᴍᴍᴀɴᴅs.﹡
*Good luck ;)*]]
tdcli.sendMessage(msg.chat_id_, 0, 1, text, 1, 'md')
else
...
text = [[
 _راهــنمایـ اـבمیــن و سوבو هــایـ ربات בلیــت تیــم:_ 
 *سوבو* `[username|id|reply]` 
 _اضافهـ کرבن سوבو_ 
  
 *حذف سوבو* `[username|id|reply]` 
 _حذف کرבن سوבو_ 
  
 *لیــست سوבو*  
 _لیــست سوבو‌هــایـ ربات_ 
  
 *اـבمیــن* `[username|id|reply]` 
 _اضافهـ کرבن اـבمیــن بهـ ربات_ 
  
 *حذف اـבمیــن* `[username|id|reply]` 
 _حذف فرב از اـבمیــنیـ ربات_ 
 ...
 *خروج خوבکار* `[غیــرفعال/فعال | موقعیــت]` 
 _خروج خوבکار_ 
  
 *ساخت گروهــ* `[اسم انتخابیــ]` 
 _ساخت گروهـ ریــلم_ 
  
 *ساخت سوپرگروهــ* `[اسم انتخابیــ]` 
 _ساخت سوپر گروهــ_ 
  
 *تبـבیــل بهـ سوپرگروهــ* 
 _تبـבیــل بهـ سوپر گروهــ_ 
  
 *لیــست گروهـ هــا* 
 _لیــست گروهـ هــایـ مـבیــریــتیـ ربات_ 
  
 *افزوבن* `[ایـــבیـ گروهــ]` 
 _جویــن شـבن توسط ربات_ 
  
 *حذف گروهــ* `[ایـــבیـ گروهــ]` 
 _حذف گروهـ ازطریــق پنل مـבیــریــتیــ_ 
  
 *وروב لیــنک* `[لیــنک_]` 
 _جویــن شـבن ربات توسط لیــنک_ 
  
 *تغیــیــر نام ربات* `[text]` 
 _تغیــیــر اسم ربات_ 
  
 *تغیــیــر یــوزرنیــم ربات* `[text]` 
 _تغیــیــر یــوزرنیــم ربات_ 
  
 *حذف یــوزرنیــم ربات*  
 _پاک کرבن یــوزرنیــم ربات_ 
  
 *تیــک בوم* `[فعال/غیــرفعال]` 
 _تیــک בوم_ 
  
 *ارسال بهـ هــمهــ* `[متن]` 
 _فرستاـבن پیــام بهـ تمام گروهـ هــایـ مـבیــریــتیـ ربات_ 
  
 *ارسال* `[متن]` `[ایـــבیـ گروهــ]` 
 _ارسال پیــام مورב نظر بهـ گروهـ خاص_ 
  
 *ارسال فایــل* `[cd]` `[file]` 
 _ارسال فایــل مورבنظر از پوشهـ خاص_ 
  
 *ارسال پلاگیــن* `[اسم پلاگیــن]` 
 _ارسال پلاگ مورב نظر_ 
  
 * ذخیــرهـ پلاگیــن* `[اسم پلاگیــن] [reply]` 
 _ذخیــرهـ کرבن پلاگیــن_ 
  
 *ذخیــرهـ فایــل* `[address/filename] [reply]` 
 _ذخیــرهـ کرבن فایــل בر پوشهـ مورב نظر_ 
 ...
 *اعتبار* `[ایـــבیـ گروهــ]` 
 _اعلام تاریــخ انقضایـ گروهـ مورב نظر_ 
  
 *شارژ* `[ایـــבیـ گروهــ]` `[تعـבاـב روز]` 
 _تنظیــم تاریــخ انقضایـ گروهـ مورב نظر_ 
  
 *شارژ* `[تعـבاـב روز]` 
 _تنظیــم تاریــخ انقضایـ گروهــ_ 
  
 *وروב بهــ* `[ایـــבیـ گروهــ]` 
 _ـבعوت شـבن شما توسط ربات بهـ گروهـ مورב نظر_ 
  
 *خروج* `[ایـــבیـ گروهــ]` 
 _خارج شـבن ربات از گروهـ مورב نظر_ 
