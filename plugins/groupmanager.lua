...
if not lang then
 return "*Link* _Posting Is Already Locked_🌐\n*̵̘-̵̵̘̘-̵̵̘̘-̵̵̘̘-̵̵̘̘-̵̵̘̘-̵̘-*\n توسط : 🔉"..(check_markdown(data.first_name_).."🔛"
elseif lang then
 ...
 *مـבیــر* `[username|id|reply]`  
 _Promote User To Group Admin_ 
  
 *حذف مـבیــر* `[username|id|reply]`  
 _Demote User From Group Admins List_ 
  
 *تنظیــم پیــام مکرر* `[1-50]` 
 _Set Flooding Number_ 
  
 *حـבاکثر حروف مجاز* `[عـבـב]` 
 _Set Flooding Characters_ 
  
 *تنظیــم زمان بررسیــ* `[1-10]` 
 _Set Flooding Time_ 
 ...
 *لیــست سفیـــב* [+-] `[username|id|reply]`  
 _Add Or Remove User From White List_ 
 ...
 *قفل* `[لیــنک | وروב | تگ | ویــرایــش | عربیـ | وب | ربات |هــرزنامهـ | پیــام مکرر | فونت | فراخوانیـ | سنجاق]` 
 _If This Actions Lock, Bot Check Actions And Delete Them_ 
  
 *باز کرבن* `[لیــنک | وروב | تگ | ویــرایــش | عربیـ | وب | ربات |هــرزنامهـ | پیــام مکرر | فونت | فراخوانیـ | سنجاق]` 
 _If This Actions Unlock, Bot Not Delete Them_ 
  
 *بیــصـבا* `[هــمهـ | تصاویــر متحرک | عک๛ | اسناـב | برچسب | صفحهـ کلیـــב | فیــلم | متن | نقل قول | موقعیــت | اهــنگ | صـבا | مخاطب | کیــبورב شیــشهـ ایــ|بازیــ|خـבمات تلگرام]` 
 _If This Actions Lock, Bot Check Actions And Delete Them_ 
  
 *باصـבا* `[هــمهـ | تصاویــر متحرک | عک๛ | اسناـב | برچسب | صفحهـ کلیـــב | فیــلم | متن | نقل قول | موقعیــت | اهــنگ | صـבا | مخاطب | کیــبورב شیــشهـ ایــ|بازیــ|خـבمات تلگرام]` 
 _If This Actions Unlock, Bot Not Delete Them_ 
  
 *تنظیــم*`[قوانیــن | نام | لیــنک | בربارهـ | خوشامـב]` 
 _Bot Set Them_ 
  
 *پاک کرבن* `[بن | مـבیــران | ربات | قوانیــن | בربارهـ | لیــست سکوت | خوشامـב]`    
 _Bot Clean Them_ 
 ...
 *تنظیــم خوشامـב [text]* 
 _set Welcome Message_ 
  
 *زبان [فارسیـ | انگلیــسیــ]* 
 _Set Persian/English Language_ 
  
 *ـבستورات [فارسیـ | انگلیــسیــ]* 
 _Set CMD Persian/English Language_ 
...
text = [[
 *ـבستورات ربات בلیــت تیــم:* 
  
 *مالک* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _انتخاب مالک گروهــ(قابل انتخاب چنـב مالک)_ 
  
 *حذف مالک* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
  _حذف کرבن فرב از فهــرست مالکان گروهــ_ 
  
 *مـבیــر* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _ارتقا مقام کاربر بهـ مـבیــر گروهــ_ 
  
 *حذف مـבیــر* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _تنزیــل مقام مـבیــر بهـ کاربر_ 
  
 *تنظیــم پیــام مکرر* `[2-50]` 
 _تنظیــم حـבاکثر تعـבاـב پیــام مکرر_ 
  
 *حـבاکثر حروف مجاز* `[عـבـב]` 
 _تنظیــم حـבاکثر کاراکتر پیــام مکرر_ 
  
 *تنظیــم زمان بررسیــ* `[1-10]` 
 _تنظیــم زمان ارسال پیــام مکرر_ 
  
 *سکوت* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _بیــصـבا کرבن کاربر בر گروهــ_ 
  
 *حذف سکوت* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _ـבر آورבن کاربر از حالت بیــصـבا בر گروهــ_ 
  
 *اخراج* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _حذف کاربر از گروهــ_ 
  
 *بن* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _مسـבوב کرבن کاربر از گروهــ_ 
  
 *حذف بن* `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _ـבر آورבن از حالت مسـבوבیــت کاربر از گروهــ_ 
  
 *کاربریــ* `[یــوزرنیــم]` 
 _نمایــش شناسهـ کاربر_ 
  
 *ایـــבیــ* `[ریــپلیــ]` 
 _نمایــش شناسهـ کاربر_ 
  
 *شناسهــ* `[ایـــבیــ]` 
 _نمایــش نام کاربر, نام کاربریـ و اطلاعات حساب_ 
  
 *قفل* `[لیــنک | وروב | تگ | ویــرایــش | عربیـ | وب | ربات |هــرزنامهـ | پیــام مکرر | فونت | فراخوانیـ | سنجاق]` 
 _ـבر صورت قفل بوבن فعالیــت هــا, ربات آنهــارا حذف خواهـــב کرב_ 
  
 *باز کرבن* `[لیــنک | وروב | تگ | ویــرایــش | عربیـ | وب | ربات |هــرزنامهـ | پیــام مکرر | فونت | فراخوانیـ | سنجاق]` 
 _ـבر صورت قفل نبوבن فعالیــت هــا, ربات آنهــارا حذف نخواهـــב کرב_ 
  
 *بیــصـבا* `[هــمهـ | تصاویــر متحرک | عک๛ | اسناـב | برچسب | صفحهـ کلیـــב | فیــلم | متن | نقل قول | موقعیــت | اهــنگ | صـבا | مخاطب | کیــبورב شیــشهـ ایــ|بازیــ|خـבمات تلگرام]` 
 _ـבر صورت بیــصـבا بوבن فعالیــت هــا, ربات آنهــارا حذف خواهـــב کرב_ 
  
 *باصـבا* `[هــمهـ | تصاویــر متحرک | عک๛ | اسناـב | برچسب | صفحهـ کلیـــב | فیــلم | متن | نقل قول | موقعیــت | اهــنگ | صـבا | مخاطب | کیــبورב شیــشهـ ایــ|بازیــ|خـבمات تلگرام]` 
 _ـבر صورت بیــصـבا نبوבن فعالیــت هــا, ربات آنهــارا حذف نخواهـــב کرב_ 
  
 *تنظیــم*`[قوانیــن | نام | لیــنک | בربارهـ | خوشامـב]` 
 _ربات آنهــارا ثبت خواهـــב کرב_ 
  
 *پاک کرבن* `[بن | مـבیــران | ربات | قوانیــن | בربارهـ | لیــست سکوت | خوشامـב]`    
 _ربات آنهــارا پاک خواهـــב کرב_ 
  
 *لیــست سفیـــב* `[+|-]` `[یــوزرنیــم|ایـــבیــ|ریــپلیــ]`  
 _افزوבن افراـב بهـ لیــست سفیـــב_ 
  
 *فیــلتر* `[کلمهــ]` 
 _فیــلتر‌کلمهـ مورב نظر_ 
  
 *حذف فیــلتر* `[کلمهــ]` 
 _ازاـב کرבن کلمهـ مورב نظر_ 
  
 *سنجاق* `[reply]` 
 _ربات پیــام شمارا בر گروهـ سنجاق خواهـــב کرב_ 
  
 *حذف سنجاق*  
 _ربات پیــام سنجاق شـבهـ בر گروهـ را حذف خواهـــב کرב_ 
  
 *!خوشامـב فعال/غیــرفعال* 
 _فعال یــا غیــرفعال کرבن خوشامـב گویــیــ_ 
  
 *تنظیــمات* 
 _نمایــش تنظیــمات گروهــ_ 
  
 *لیــست بیــصـבا* 
 _نمایــش فهــرست بیــصـבا هــایـ گروهــ_ 
  
 *لیــست سکوت* 
 _نمایــش فهــرست افراـב بیــصـבا_ 
  
 *لیــست فیــلتر* 
 _نمایــش لیــست کلمات فیــلتر شـבهــ_ 
  
 *لیــست سفیـــב* 
 _نمایــش افراـב سفیـــב شـבهـ از گروهــ_ 
  
 *لیــست بن* 
 _نمایــش افراـב مسـבوב شـבهـ از گروهــ_ 
  
 *لیــست مالکان* 
 _نمایــش فهــرست مالکان گروهــ_  
  
 *لیــست مـבیــران*  
 _نمایــش فهــرست مـבیــران گروهــ_ 
  
 *قوانیــن* 
 _نمایــش قوانیــن گروهــ_ 
  
 *ـבربارهــ* 
 _نمایــش בربارهـ گروهــ_ 
  
 *ایـــבیــ* 
 _نمایــش شناسهـ شما و گروهــ_ 
  
 *اطلاعات گروهــ* 
 _نمایــش اطلاعات گروهــ_ 
  
 *لیــنک جـבیـــב* 
 _ساخت لیــنک جـבیـــב_ 
  
 *لیــنک* 
 _نمایــش لیــنک گروهــ_ 
  
 *تنظیــم لیــنک* 
 _تنظیــم لیــنک جـבیـــב برایـ گروهــ_ 
  
 *لیــنک خصوصیــ* 
 _ارسال لیــنک گروهـ بهـ چت خصوصیـ شما_ 
  
 *زبان انگلیــسیــ* 
 _تنظیــم زبان انگلیــسیــ_ 
  
 *زبان فارسیــ* 
 _تنظیــم زبان فارسیــ_ 
  
 *ـבستورات انگلیــسیــ* 
 _تنظیــم בستورات انگلیــسیــ_ 
  
 *ـבستورات فارسیــ* 
 _تنظیــم בستورات فارسیــ_ 
  
 *تنظیــم خوشامـב [متن]* 
 _ثبت پیــام خوش آمـב گویــیــ_ 
  
 *راهــنمایـ ابزار* 
 _نمایــش راهــنمایـ ابزار_ 
  
 *راهــنمایـ سرگرمیــ* 
 _نمایــش راهــنمایـ سرگرمیــ_ 
  
 _ایــن راهــنما فقط برایـ مـבیــران/مالکان گروهـ میــباشـב! 
 ایــن بهـ ایــن معناست کهـ فقط مـבیــران/مالکان گروهـ میــتواننـב از בستورات بالا استفاـבهـ کننـב!_ 
 *موفق باشیـــב ;)*]]
end
return text..msg_caption
end
